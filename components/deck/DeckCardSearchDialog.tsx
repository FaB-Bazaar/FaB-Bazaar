...
// // components/deck/DeckCardSearchDialog.tsx - Dedicated dialog for adding cards to decks
// "use client"
...
//   onOpenChange: (open: boolean) => void
//   onSelectCard: (card: any, printing: any, quantity: number) => void
//   targetCategory: 'hero' | 'equipment' | 'main' | 'sideboard'
//   deckFormat?: string
//   currentDeck?: {
//     printings: any[]
//     format: string
...
//     // Add format legality filter - commented out for debugging
//     // if (deckFormat) {
//     //   params.append('format', deckFormat.toLowerCase().replace(' ', '_'));
//     // }
...
//   // Check if card is appropriate for target category
//   const isCardValidForCategory = (card: any): { valid: boolean; reason?: string } => {
//     if (!card.types || !Array.isArray(card.types)) {
//       return { valid: true }; // Allow if we can't determine type
//     }
...
//   // Check quantity limits for FAB
//   const getMaxQuantityForCard = (card: any): number => {
//     if (!card.types || !Array.isArray(card.types)) return 3;
...
//     // Most cards in FAB have a limit of 3
//     return 3;
//   };

//   // Get current quantity of this card in deck
//   const getCurrentQuantityInDeck = (card: any): number => {
//     if (!currentDeck?.printings || !card.unique_id) return 0;
    
//     return currentDeck.printings.filter(p => 
//       p.printingDetails?.card_unique_id === card.unique_id && 
...
//                 <Search className="absolute left-2.5 top-2.5 h-4 w-4 text-gray-500" />
//                 <Input
//                   placeholder={`Search for ${targetCategory} cards... (e.g., 'red action under 10' or 'majestic weapon')`}
//                   className="pl-8 pr-10"
//                   value={searchQuery}
//                   onChange={(e) => setSearchQuery(e.target.value)}
//                 />
...
//                           selectedCard?.unique_id === card.unique_id 
//                             ? "border-blue-500 bg-blue-50 dark:bg-blue-950 dark:border-blue-400" 
//                             : validation.valid && canAdd
//                               ? "hover:bg-gray-50 dark:hover:bg-gray-700" 
//                               : "opacity-50 cursor-not-allowed"
//                         }`}
//                         onClick={() => validation.valid && canAdd && handleCardSelect(card)}
//                       >
//                         <CardContent className="p-3">
//                           <div className="flex justify-between items-start">
//                             <div className="flex-1">
...
//                 <div className="text-center py-8">
//                   <Search className="h-8 w-8 text-gray-400 mx-auto mb-2" />
//                   <p className="text-gray-500 dark:text-gray-400">Search for cards to add to your {targetCategory}</p>
//                   <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
//                     Only cards valid for {targetCategory} will be shown
//                   </p>
//                 </div>
//               )}
//             </div>
...
//                   >
//                     <Plus className="h-4 w-4 mr-2" />
//                     Add {quantity}x to {targetCategory}
//                   </Button>
//                 </div>
//               </>
//             ) : (
