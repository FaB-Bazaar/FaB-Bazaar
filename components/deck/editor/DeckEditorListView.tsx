          ...
          <>
            ...
            <div className="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-gray-900 rounded-t-2xl shadow-2xl">
              ...
              <div className="py-1">
                ...
                <button type="button" className="w-full text-left px-5 py-2.5 text-sm text-gray-800 dark:text-gray-200 active:bg-gray-100 dark:active:bg-gray-800 transition-colors" onClick={() => { onSwap({ printingId: primary.printingId, cardUniqueId: primary.printingDetails?.card_unique_id || '', cardName: group.displayName, category }); dismiss(); }}>
                  Swap printing
                </button>
                {onMove && moveDests.map(({ to, label }) => (
                  <button key={to} type="button" className="w-full text-left px-5 py-2.5 text-sm text-gray-800 dark:text-gray-200 active:bg-gray-100 dark:active:bg-gray-800 transition-colors" onClick={() => { onMove(primary.printingId, category, to, qty); dismiss(); }}>
                    {label}
                  </button>
                ))}
            ...
            >
              ...
              {(!isSpecial && (onRemoveTile || onAddTile) || onAddToBinder || onAddToWants) && (
                <div className="flex shrink-0 items-center justify-center gap-4 px-5 py-2 border-b border-gray-100 dark:border-gray-800">
                  {!isSpecial && (onRemoveTile || onAddTile) && (
                    <div className="flex items-center gap-4">
                      <button
                        ...
                        aria-label={`Remove one ${tile.name}`}
                      ...
                      >−</button>
                      <span className="text-base font-bold text-gray-900 dark:text-white w-6 text-center tabular-nums">
                        {section.tiles.filter(t => t.cardUniqueId === tile.cardUniqueId).length}
                      </span>
                      <button
                        type="button"
                        disabled={!onAddTile}
                        aria-label={`Add one ${tile.name}`}
                  ...
                  {(onAddToBinder || onAddToWants) && (
                    <>
                      <span className="h-7 w-px bg-gray-200 dark:bg-gray-700" aria-hidden="true" />
                      {onAddToBinder && (
                        <button
                          type="button"
              ...
              <div className="shrink-0 py-1">
                ...
                {moveDests.map(({ to, label }) => (
                  <button key={to} type="button" className="w-full text-left px-5 py-2.5 text-sm text-gray-800 dark:text-gray-200 active:bg-gray-100 dark:active:bg-gray-800 transition-colors" onClick={() => { onMoveTo!(tile, to); dismiss(); }}>
                    {label}
                  </button>
                ))}
              </div>
