// Client service for the hosted Volzar chat (superadmins + paid supporters).
// Streams AgentEvents from POST /api/volzar via fetch + reader
...
export async function streamChat(opts: {
  ...
  try {
    const response = await fetch('/api/volzar', {
      method: 'POST',
      credentials: 'include',
      signal: opts.signal,
...
export async function resolveConfirmation(opts: {
  ...
  try {
    const response = await fetch('/api/volzar/confirm', {
      method: 'POST',
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
