...
| `scanClient` | `scan-client.ts` | Card scanner: `identifyCard(blob)` multipart → `/api/scan/identify`; phone pairing `createSession` / `pairSession` / `identifyCardForSession` / `sessionEventsUrl` (SSE) |
