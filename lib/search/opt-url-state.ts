...
export interface OptUiState {
  ...
  /** "Your collection": restrict to printings the signed-in user owns. */
  ownedOnly: boolean;
...
export function uiStateToParams(s: OptUiState): URLSearchParams {
  ...
  if (s.healthMin) p.set('healthMin', s.healthMin);
  if (s.healthMax) p.set('healthMax', s.healthMax);
...
export function paramsToUiState(p: URLSearchParams): Partial<OptUiState> {
  ...
  if (p.get('owned') === '1') out.ownedOnly = true;
  ...
  setStr('healthMin', 'healthMin'); setStr('healthMax', 'healthMax');
