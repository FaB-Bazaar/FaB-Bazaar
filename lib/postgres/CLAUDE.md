...
| `printing_image_hashes` | Card-scanner perceptual hashes, one row per printing (16-char hex pHash + dHash + `art_phash` of the fixed art rectangle, `image_url` hashed). Loaded from the fab-cube dataset by `scripts/import-dataset-hashes.ts` (+ `compute-image-hashes.ts` for unanchored printings), never by the pipeline. `dhash` NULL on imported rows. Migrations 0109/0110/0111 | `printings.printingId` CASCADE |
...
| `market_feed_listings` | Curated anonymous daily buy/sell prices from Facebook groups (`/feed`). Written only by the superadmin `submit_market_feed` MCP tool → `POST /api/feed`, which REPLACES a whole `feed_date`. No poster names by design; `post_url` (0116) links the Facebook post — https Facebook hosts only, tracking params stripped by the service. `card_unique_id` NULL = unmatched or ambiguous name. `side` 'trade' = wanted in exchange (price NULL allowed only there) and `variant` (Marvel / Extended Art / …) added in 0117. `region` ('na'/'eu'/'apac', 0118): a submission replaces one (feed_date, region). Migrations 0115–0118 | `cards` SET NULL, `users.id` SET NULL (`created_by`) |
...
- After a prod-data refresh, the `PostgresSetsService` orphan-code test fails when printings reference sets newer than the seeded `sets` rows — data drift, not code. Add the missing `sets` rows, then regenerate constants.
...
- Source anchors (0088): `printings.fab_cube_printing_id/lss_print_id/lss_print_code`, `cards.fab_cube_card_id`. Anchors are upstream references — internal PKs NEVER change; reconciliation stamps anchors in place (pipeline 005 `_reconcile_anchors` is the only writer on existing rows). `fab_cube_* IS NULL` = provisional.
