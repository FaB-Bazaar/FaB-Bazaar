...
| `scanService` | Card scanner: `upsertHashes`/`listHashIndex` over `printing_image_hashes` + `identify(hashPair, {limit, pitchHint})` — loads the whole index into memory (10-min TTL, `clearIndexCache()` in tests), ranks by Hamming distance, groups by card name → pitch → ALL printings of the card (unhashed ones carry `distance: null`) |
