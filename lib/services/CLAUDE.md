...
| `marketFeedService` | Anonymous daily market feed (`market_feed_listings`). `replaceDay` validates + resolves each listing to one card (collector number, else name + pitch; ambiguous → unmatched, reported back) and swaps the day in one transaction; `getDay` joins the cheapest English `tcg_low` for the listing's foiling/collector number. "Today" = US Eastern (`lib/market-feed/feed-date.ts`). Variant pricing: Marvel = `rarity='v'`, other variants = `art_variations` code. `getViewerMatches(userId, date)` returns the viewer's owned copies (quantity, for-trade, foilings, variants) + wants for that day's cards — only ever for the signed-in viewer; `lib/market-feed/personalize.ts` turns it into /feed badges and trade-post summaries. Every read/write is per region (`na`/`eu`/`apac`, default `na`); /feed resolves the viewer's region via `resolveFeedRegion` (`?region` → `feed_region` cookie → profile `country_code` → Cloudflare `CF-IPCountry` → na) |
