...
export function systemPrompt(username: string): string {
  return [
    ...
    `Talishar. Your search QUERIES must always use ENGLISH card names (translated`,
    `names are not searchable) — translate the user's card reference to its`,
    `English name before searching. A result marked "no XX printing exists" means`,
    `the card is only printed in English — say so briefly when it matters.`,
