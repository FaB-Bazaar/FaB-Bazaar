...
export function systemPrompt(username: string): string {
  return [
    ...
    `1. Start from the hero's curated kit pools: list_curated_lists({heroName}),`,
    `then get_curated_list for the lists matching the deck's format. Those are a`,
    `curator's real, legal picks to build around — quote them directly.`,
