...
// add_card_tag; same route (DELETE), same id contract, same scope semantics.
// It does NOT retract community votes — those belong to their voters.
import { callAssignRoute, CARD_TAG_SCOPES } from './addCardTag';
...
Inverse of add_card_tag — same IDs (cardUniqueId = card_unique_id, NOT a printing_id; tag = fab://facet-tags slug) and the same scope semantics ('name' = all same-name pitch variants, default; 'card' = this exact card only).
