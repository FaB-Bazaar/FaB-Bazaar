...
- **Turn log card images — tokens & removed cards** — Cards like `gold` (Gravy Bones token) are never in `deck.maindeck/equipment/hero`, so the slug lookup misses them. Cards removed from the deck since a game was played also fail. `DeckResultsTab` handles this via `fallbackCardImages` state + a `useEffect` calling `/api/printings/images` for any player turn log cardIds still missing after the deck-based passes.
