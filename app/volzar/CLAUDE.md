...
- **Dialog adds must refresh open tables** — CardSearchDialog adds write server-side, so `refreshItemsForTarget` re-drills the affected binder/deck/wants after each successful add and swaps fresh content into every matching item (uid-preserving `refreshDataItem`; seq guard drops out-of-order responses). Without it the workspace looks like the add was a no-op. The row ±/swap buttons instead update optimistically in place.
