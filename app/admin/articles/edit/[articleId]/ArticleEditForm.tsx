...
import { Loader2, Save, Plus, X, Eye, RefreshCw, ArrowLeft, ChevronUp, ChevronDown, Search } from 'lucide-react';
import {
  Select,
  SelectContent,
  SelectItem,
...
import { CalloutSectionEditor } from './CalloutSectionEditor';
import { OpportunityCardSectionEditor } from './OpportunityCardSectionEditor';
import { SpotlightCardSectionEditor } from './SpotlightCardSectionEditor';
import { IntroSectionEditor } from './IntroSectionEditor';
import { BylineSectionEditor } from './BylineSectionEditor';
...
import { updateArticle, revalidateArticle } from '@/app/actions/articleActions';
...
export function ArticleEditForm({ initialData }: { initialData: any }) {
  const [isRevalidating, startRevalidationTransition] = useTransition();
  ...
  const addSection = (type: 'text' | 'card-carousel' | 'video' | 'creator-spotlight' | 'callout' | 'opportunity-card' | 'spotlight-card' | 'intro' | 'byline' | 'section-header' | 'key-takeaways' | 'match-report' | 'decklist-block' | 'buylist-block', index?: number) => {
    ...
    if (type === 'video') {
        ...
        newSection.creatorName = "";
        newSection.creatorUrl = "";
    }
  ...
  const handleSubmit = async () => {
    startTransition(async () => {
      const result = await updateArticle(article._id, article);
      if (result.success) {
  ...
  const handleRevalidate = () => {
    startRevalidationTransition(async () => {
      const result = await revalidateArticle(article.publicId, article.contentType);
      if (result.success) {
        toast({ title: "Cache Updated!", description: "The live page has been updated." });
      } else {
        toast({ title: "Cache Update Failed", description: result.error, variant: "destructive" });
      }
    });
  };
    ...
    <div className="space-y-6 pb-24">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Enter the core details for your article.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Input value={article.title} onChange={(e) => handleFieldChange('title', e.target.value)} placeholder="Title"/>
          <Input value={article.subtitle || ''} onChange={(e) => handleFieldChange('subtitle', e.target.value)} placeholder="Subtitle"/>
          <Input value={article.slug} onChange={(e) => handleFieldChange('slug', e.target.value)} placeholder="URL Slug"/>
          <Select value={article.status} onValueChange={(value) => handleFieldChange('status', value)}>
          ...
          <div>
            ...
            <Select value={article.contentType} onValueChange={(value) => handleFieldChange('contentType', value)}>
      ...
      {article.sections.length === 0 && <AddSectionWidget insertIndex={0} />}
              ...
              <CardContent>
                {section.type === 'text' ? (
                  <MdxEditor
                    ...
                    onChange={(newContent) => handleSectionChange(index, { content: newContent })}
                  />
                  ...
                  <CarouselSectionEditor
                    cards={section.cards || []}
                    onChange={(cards) => handleSectionChange(index, { cards })}
                  />
                ) : section.type === 'video' ? (
                  <VideoSectionEditor
                    section={section}
      ...
      <div className="flex justify-between items-center sticky bottom-0 py-4 bg-background/80 backdrop-blur-sm z-10 border-t">
        ...
        <div className="flex gap-2">
          <Button
            ...
            disabled={isRevalidating || article.status !== 'published'}
            variant="secondary"
            title={article.status !== 'published' ? "Article must be published to update the live page" : "Push current saved changes to the live site"}
          >
            {isRevalidating ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <RefreshCw className="h-4 w-4 mr-2" />}
            Update Live Page
          </Button>
