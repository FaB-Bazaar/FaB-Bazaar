...
- **Bad upstream feed data → feed_overrides, never DB edits** — when the fab-cube feed ships a wrong `tcgplayer_product_id`/url (SEA015-017 priced off the 1st Strike products this way), record a row in `feed_overrides` (`/admin/feed-overrides`, superadmin). Pipeline step 02 patches the feed before pricing, so the correction survives every nightly run; direct `printings` edits are clobbered by 005/006 and never reprice. See pipeline/CLAUDE.md.
