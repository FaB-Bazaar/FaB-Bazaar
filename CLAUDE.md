...
- **Two shorthand parsers, edit BOTH** — `lib/fab-shorthand-parser.ts` (MCP) and `lib/search/fab-shorthand-parser.ts` (/opt) drift; shared tests in `lib/fab-shorthand-parser.test.ts` pin them. A pattern's parser may `return false` to DECLINE a match (token stays in the text for the name search) — without it the token is blanked even when unapplied (the "red alert boots" → "alert boots" bug). `c:`/`tal:`/`hero:` accept aliases + unambiguous prefixes (`c:mech`, `tal:dra`, `hero:dor`) via `resolveClassShorthand` (`lib/fab-constants/classes.ts`), `TalentUtils.resolveTalent`, `resolveHeroShorthand` (`lib/fab-constants/heroes.ts`); ambiguous/unknown input stays raw on purpose — add curated aliases to `CLASS_ALIASES` / `TALENT_ABBREVIATIONS` / `HERO_NICKNAMES`, don't loosen the uniqueness rule.
