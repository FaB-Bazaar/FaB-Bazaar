...
- **Two shorthand parsers, edit BOTH** — `lib/fab-shorthand-parser.ts` (MCP) and `lib/search/fab-shorthand-parser.ts` (/opt) drift; shared tests in `lib/fab-shorthand-parser.test.ts` pin them. A pattern's parser may `return false` to DECLINE a match (token stays in the text for the name search) — without it the token is blanked even when unapplied (the "red alert boots" → "alert boots" bug); both loops honour it (ported to the /opt parser 2026-09 for the bare `-generic` / `!gen` class-exclusion token, which declines anything the class resolver can't place). `c:`/`tal:`/`hero:` accept aliases + unambiguous prefixes (`c:mech`, `tal:dra`, `hero:dor`) via `resolveClassShorthand` (`lib/fab-constants/classes.ts`), `TalentUtils.resolveTalent`, `resolveHeroShorthand` (`lib/fab-constants/heroes.ts`); every two-letter class prefix resolves (`me`→mechanologist is the one curated tie-break; merchant is `mer`); ambiguous/unknown input stays raw on purpose — add curated aliases to `CLASS_ALIASES` / `TALENT_ABBREVIATIONS` / `HERO_NICKNAMES`, don't loosen the uniqueness rule.
...
- **Multi-class card legality: subset unless the type box has a slash** — CR 1.1.3b / 2.14.1b. "Pirate Necromancer Action" (no slash) is ONE supertype set, so the hero needs EVERY class (`classes <@ hero`): Malice (Necromancer) can't play Angry Bones, Gravy Bones can. "Brute / Guardian Instant" is a HYBRID: legal if EITHER set fits (`classes && hero`). `cards.type_text` keeps the slash and is the discriminator in the precise hero-legality check (`PostgresPrintingsService`, `heroClasses` branch). A March 2026 commit flipped the whole check to overlap "so ranger+assassin cards show for assassins" — that was only right for the slash hybrids and leaked all 55 Pirate Necromancer cards into every Necromancer pool. Tests: `PostgresPrintingsService.hybridClass.test.ts`. NB `/api/printings/search` (what the Add Card dialog and useCardSearch post to — NOT `/api/search/core`) caches responses in Redis for 24h keyed by body hash + `SEARCH_CACHE_VERSION`, invalidated only by a price update — bump the version whenever a change alters which rows a body returns, or prod keeps serving the old answer for every already-seen body while fresh curls look fixed.
...
- **Future Classic Constructed (2026-09)** — deck format `'Future Classic Constructed'` (search/API code `future_cc`, aliases `fcc`/`future cc`) = today's CC pool PLUS every card with a printing in a set whose `sets.release_date > CURRENT_DATE` (category ≠ `excluded`, NULL date = never), minus the CC banlist + `cc_suspended`; adult heroes only, 3-of. The DB is the only membership source — the fab-cube feed flips `cc_legal` on release, so unreleased cards are `cc_legal=false` (AMA/MPA/AMO at the time of writing) and a set enters the pool the moment its `sets` row carries a release date. The ONE predicate is `isFutureReleaseCard` (`lib/services/postgres/future-release.ts`), used by the search format filter, `listHeroCards`, the deck add-card gate (`futureCcLegal` in `validation.ts`) and the `future_release` search-row flag that `deckLegalityVerdict` reads for the card lightbox. Adding another format means touching every display-name→code map at once: `FORMAT_CODES`, `DECK_FORMAT_TO_API_CODE`, `deckFormatToSnake` + `FORMAT_LEGAL_FIELD`, `FORMAT_HERO_REQUIREMENT`, `HERO_FORMATS`, `useDeckEditor` `FORMAT_TO_SEARCH`, `deckFormatToBannedFormat`, `useExcludedHeroes`, both Talishar `FORMAT_MAP`s (Future CC exports as `cc`), `DeckSettings`/community/curation `FORMATS`, the MCP enums (`createDeck`, `searchPrintings`, `searchCapabilities`, `fabConstants`, `heroesByFormat`) and `FORMAT_OPTIONS` for /opt. `cards.types` drives the young/adult split everywhere (`deriveFormatFromHero`, hero list); the hero list is a CC superset (whatever the feed flags `cc_legal` stays) and only the future leg is adult-only. Local-DB trap: `heroLegality.test.ts` toggles `young` on a real hero — its snapshot now restores `types` too; if adult heroes show up typed `young` locally, run `UPDATE cards SET types = array_remove(types,'young') WHERE 'young' = ANY(types) AND type_text NOT ILIKE '%young%'` (prod was verified clean).
